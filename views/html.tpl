...
$(document).ready(function(){
    ...
    */
    ...
    $('#mainDiv').append('<p><h3>Find Hospitals based on Admission Code or Procedure Code</h3></p><br>');
    $('#mainDiv').append(rbtnAdmission);
    $('#mainDiv').append(rbtnProcedure);
	$('#mainDiv').append('<br><br>');
    var selectBox =  $("<select class='form-control' class='form-control' style='width:20%'></select>").attr("id", 'selectDropdown').attr('multiple','multiple');
	...
	var tbZipCode = $('<input id="zipcode" autocomplete="off" type="text" class="form-control" placeholder="Zipcode" style="width:20%">');
    $('#mainDiv').append('<br><br>');
...
function getHospitals()
{
    ...
    //document.getElementById("getHospitals").onclick = function () {
...
function displayHospitalList(data)
{
   ...
   // console.log(data);
	location.href = "/hospitalsListNew" ;
...
function displayAQoC(data)
{
    ...
    $.each(data, function(i,record){	
		...
		if(record['Inhospital_disease'] > 20){		
		L.circle([record['Latitude'],record['Longitude']],8,{
    color: 'red',
    fillColor: '#f03',
    fillOpacity: 0.5
...
function displayACoC(data)
{
    ...
    $.each(data, function(i,record){	
		...
		if(record['Avg_Tot_Charges'] > 20){		
		L.circle([record['Latitude'],record['Longitude']],8,{
    color: 'red',
    fillColor: '#f03',
    fillOpacity: 0.5
}).addTo(map).bindPopup("<b>Avg_Tot_Charges:</b><br>"+record['Avg_Tot_Charges']).openPopup();
    }
...
function displayALoS(data)
{
    ...
    $.each(data, function(i,record){	
		...
		if(record['Avg_LOS'] > 20){		
		L.circle([record['Latitude'],record['Longitude']],8,{
    color: 'red',
    fillColor: '#f03',
    fillOpacity: 0.5
}).addTo(map).bindPopup("<b>Avg_LOS:</b><br>"+record['Avg_LOS']).openPopup();
    }
...
Array.prototype.min = function() {
  return Math.min.apply(null, this);
    ...
    $.each(data, function(i,record){	
		...
		if(record['Inhospital_disease'] > min){		
		L.circle([record['Latitude'],record['Longitude']],8,{
    color: 'red',
    fillColor: 'red',
    fillOpacity: 0.5,
		...
		if(med <= record['Inhospital_disease'] <= good){		
		L.circle([record['Latitude'],record['Longitude']],8,{
    color: 'blue',
    fillColor: '#f03',
    fillOpacity: 0.5
		...
		if(record['Inhospital_disease'] < med){		
		L.circle([record['Latitude'],record['Longitude']],8,{
    color: 'green',
    fillColor: '#f03',
    fillOpacity: 0.5
...
function displayprocHospitalList(data)
{
    ...
    $('#mainDiv').find('#hospital_list').remove();
    var table_tag = $('<table class="table table-striped id="hospital_list"></table>')
...
<body>
  ...
  <div>
  <img src="./static/logo.png" alt="Logo" title="Intelligent" width="100" height="70px" border="10px">
     <img src="./static/logo2.png" alt="Logo" title="MediQoc" align="left" width="100" height="70px" border="10px">
      </div>
      <center>
